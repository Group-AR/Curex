...
# -----------------------------------------------------------------------------
# الهوية البصرية (خلفية خضراء طبية مع صورة خلفية استشفائية)
# -----------------------------------------------------------------------------
st.markdown("""
    <style>
    ...
    .stApp {
        background: linear-gradient(rgba(3, 20, 12, 0.88), rgba(2, 12, 7, 0.92)), 
    ...
    /* واجهة الـ Hero الخضراء الساحرة */
    .hero-section {
        background: linear-gradient(135deg, rgba(6, 78, 59, 0.65) 0%, rgba(4, 120, 87, 0.45) 100%);
    ...
    /* الأزرار المتدرجة (أخضر زمردي ساحر) */
    .stButton>button {
...
# -----------------------------------------------------------------------------
# الرسوم البيانية المتناسقة مع الثيم الأخضر
# -----------------------------------------------------------------------------
def style_plot(fig, title_text):
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#F0FDF4", size=13, family="Cairo"),
        title=dict(text=title_text, x=0.5, xanchor='center', font=dict(color="#34D399", size=18, family="Cairo")),
        legend=dict(font=dict(color="#F0FDF4", size=11), x=1.02, y=0.5),
        margin=dict(t=60, b=80, l=40, r=120)
    )
    return fig

def draw_charts(df_inventory, df_trans):
    st.markdown("<br><h3 style='margin-bottom: 25px; color: #34D399;'>📊 التحليلات والتقارير الطبية المتقدمة</h3>", unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    with col1:
        if not df_trans.empty and "Date" in df_trans.columns and "Quantity" in df_trans.columns:
            fig_line = px.line(df_trans, x="Date", y="Quantity", color="Item Name" if "Item Name" in df_trans.columns else None, template="plotly_dark", markers=True, color_discrete_sequence=['#34D399', '#6EE7B7', '#10B981', '#059669'])
    ...
    with col2:
        if not df_inventory.empty:
            fig_bar = px.bar(df_inventory, x="Item Name", y="Current Balance", template="plotly_dark", color="Current Balance", color_continuous_scale=["#059669", "#34D399"])
...
def create_dashboard():
    st.markdown("""
        <div class="animated-section">
            <h1 style="font-size: 32px; margin-bottom: 10px; color: #34D399;"><i class="bi bi-speedometer2"></i> لوحة التحكم الإدارية</h1>
            <p style="color: #A7F3D0; font-size: 15px; margin-bottom: 25px;">متابعة شاملة لحالة المخزون، العمليات، والمؤشرات الحيوية لمتجر CureX.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with c1:
        st.markdown(f"""
            <div class="kpi-card-medical">
                <div style="font-size: 26px; color: #34D399; margin-bottom: 8px;"><i class="bi bi-box-seam"></i></div>
    ...
    with c2:
        st.markdown(f"""
            <div class="kpi-card-medical">
                <div style="font-size: 26px; color: #6EE7B7; margin-bottom: 8px;"><i class="bi bi-arrow-repeat"></i></div>
...
def create_store():
        ...
        <div class="hero-section animated-section">
            <div>
                <h1 style="font-size: 32px; margin-bottom: 10px;">
                    Cure<span style="color: #34D399;">X</span> للمستلزمات الطبية
                </h1>
                <p style="font-size: 15px; color: #A7F3D0; max-width: 600px;">نوفر أحدث الأجهزة والمستلزمات الطبية بأعلى معايير الدقة والجودة لدعم منظومتك الصحية.</p>
            </div>
            <div style="font-size: 45px; font-weight: 900; background: rgba(16, 185, 129, 0.2); padding: 15px 25px; border-radius: 16px; border: 1px solid rgba(52, 211, 153, 0.4); box-shadow: 0 0 20px rgba(52, 211, 153, 0.2);">
                Cure<span style="color: #34D399;">X</span> ↗
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='margin-top: 30px; margin-bottom: 20px; color: #34D399;'>المستلزمات الطبية المتاحة للطلب الفوري</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='margin-top: 20px; margin-bottom: 20px; color: #34D399;'>تفضل بملء بياناتك لإتمام الطلب</h3>", unsafe_allow_html=True)
...
st.markdown(f"""
    <div class="top-nav animated-section">
        <div style="display: flex; align-items: center; gap: 12px;">
            <span style="font-size: 22px; font-weight: 900; color: #F0FDF4;">Cure<span style="color: #34D399;">X</span> ↗</span>
            <span style="font-size: 14px; font-weight: 600; color: #A7F3D0;">منظومة إدارة المستلزمات الطبية</span>
        </div>
        <div style="font-size: 13px; color: #34D399; background: rgba(16,185,129,0.15); padding: 5px 12px; border-radius: 20px; border: 1px solid rgba(52,211,153,0.3);">
            <i class="bi bi-clock"></i> الوقت الحالي: {current_time_str}
        </div>
        <div style="display: flex; align-items: center; gap: 10px;">
            <span style="color: #34D399; font-weight: 700; font-size: 13px;"><i class="bi bi-check-circle-fill"></i> متصل</span>
        </div>
    </div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown("""
        <div style="text-align: center; padding: 15px 0;" class="animated-section">
            <div style="font-size: 34px; font-weight: 900; color: #F0FDF4; margin-bottom: 4px;">Cure<span style="color: #34D399;">X</span> ↗</div>
            <p style="font-size: 12px; color: #A7F3D0; margin-top: 2px;">Medical Supplies ERP</p>
        </div>
        <hr style="border-color: rgba(52,211,153,0.2); margin-bottom: 15px;">
    """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
st.markdown("""
    <hr style='border-color: rgba(52,211,153,0.2); margin-top: 50px;'>
    <div style='display: flex; justify-content: space-between; align-items: center; color: #A7F3D0; font-size: 13px; padding-bottom: 25px;' class='animated-section'>
        <div><strong style="color: #34D399;">CureX</strong> - نظام إدارة المستلزمات الطبية</div>
