...
# -----------------------------------------------------------------------------
# الهوية البصرية (درجات أهدأ بكثير وأكثر راحة للعين)
# -----------------------------------------------------------------------------
st.markdown("""
    <style>
    ...
    .stApp {
        background: linear-gradient(rgba(15, 23, 42, 0.94), rgba(10, 15, 30, 0.96)), 
    ...
    /* واجهة الـ Hero بدرجات هادئة وناعمة جداً */
    .hero-section {
        background: linear-gradient(135deg, rgba(30, 41, 59, 0.4) 0%, rgba(13, 148, 136, 0.1) 100%);
    ...
    .product-medical-card:hover {
        transform: translateY(-4px);
        border-color: #14B8A6;
        box-shadow: 0 12px 35px rgba(20, 184, 166, 0.1);
    }
    ...
    /* الأزرار بدرجات هادئة جداً وناعمة */
    .stButton>button {
    ...
    .stButton>button:hover {
        ...
        box-shadow: 0 10px 25px rgba(20, 184, 166, 0.25) !important;
    }
...
def style_plot(fig, title_text):
    fig.update_layout(
        ...
        font=dict(color="#CBD5E1", size=13, family="Cairo"),
        title=dict(text=title_text, x=0.5, xanchor='center', font=dict(color="#2DD4BF", size=18, family="Cairo")),
        legend=dict(font=dict(color="#CBD5E1", size=11), x=1.02, y=0.5),
...
def draw_charts(df_inventory, df_trans):
    ...
    with col1:
        if not df_trans.empty and "Date" in df_trans.columns and "Quantity" in df_trans.columns:
            fig_line = px.line(df_trans, x="Date", y="Quantity", color="Item Name" if "Item Name" in df_trans.columns else None, template="plotly_dark", markers=True, color_discrete_sequence=['#1E3A8A', '#14B8A6', '#60A5FA', '#2DD4BF'])
    ...
    with col2:
        if not df_inventory.empty:
            fig_bar = px.bar(df_inventory, x="Item Name", y="Current Balance", template="plotly_dark", color="Current Balance", color_continuous_scale=["#1E3A8A", "#14B8A6", "#2DD4BF"])
...
def create_dashboard():
    st.markdown("""
        <div class="animated-section">
            ...
            <p style="color: #94A3B8; font-size: 15px; margin-bottom: 25px;">متابعة شاملة لحالة المخزون، العمليات، والمؤشرات الحيوية لمتجر CureX.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<hr style='border-color: rgba(51,65,85,0.3); margin: 35px 0;'>", unsafe_allow_html=True)
    ...
    st.markdown("""
        <div class="hero-section animated-section">
            <div>
                ...
                <p style="font-size: 15px; color: #94A3B8; max-width: 600px;">نوفر أحدث الأجهزة والمستلزمات الطبية بأعلى معايير الدقة والجودة لدعم منظومتك الصحية.</p>
            </div>
            <div style="font-size: 45px; font-weight: 900; background: rgba(20, 184, 166, 0.1); padding: 15px 25px; border-radius: 16px; border: 1px solid rgba(45, 212, 191, 0.2); box-shadow: 0 0 15px rgba(20, 184, 166, 0.08);">
...
st.markdown(f"""
    <div class="top-nav animated-section">
        <div style="display: flex; align-items: center; gap: 12px;">
            <span style="font-size: 22px; font-weight: 900; color: #CBD5E1;">Cure<span style="color: #2DD4BF;">X</span> ↗</span>
            <span style="font-size: 14px; font-weight: 600; color: #94A3B8;">منظومة إدارة المستلزمات الطبية</span>
        </div>
        <div style="font-size: 13px; color: #2DD4BF; background: rgba(20,184,166,0.08); padding: 5px 12px; border-radius: 20px; border: 1px solid rgba(45,212,191,0.15); ">
...
with st.sidebar:
    st.markdown("""
        <div style="text-align: center; padding: 15px 0;" class="animated-section">
            <div style="font-size: 34px; font-weight: 900; color: #CBD5E1; margin-bottom: 4px;">Cure<span style="color: #2DD4BF;">X</span> ↗</div>
            <p style="font-size: 12px; color: #94A3B8; margin-top: 2px;">Medical Supplies ERP</p>
        </div>
        <hr style="border-color: rgba(51,65,85,0.3); margin-bottom: 15px;">
    """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
st.markdown("""
    <hr style='border-color: rgba(51,65,85,0.3); margin-top: 50px;'>
    <div style='display: flex; justify-content: space-between; align-items: center; color: #94A3B8; font-size: 13px; padding-bottom: 25px;' class='animated-section'>
